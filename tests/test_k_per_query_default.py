"""
k_per_query Production Default(15 -> 20) 변경 검증. 전부 Mock 기반 — 실제
Ollama 호출 없이(네트워크 연결 시도조차 없이) 결정론적으로 실행된다. (이전
10 -> 15 변경 당시의 검증도 동일한 패턴이었으며, sample_specs가 52 -> 100개로
늘어난 뒤 실측 Recall 재검증에 따라 이번에 다시 15 -> 20으로 올랐다 —
agent/spec_retriever.py::retrieve_for_requirement 및 agent/pipeline.py::
retrieve_and_generate의 docstring 참고.)
...
def test_retriever_default_is_20(patched_vector_store):
    ...
    assert all(k == 20 for k in patched_vector_store.last_k_values)
...
# ---------------------------------------------------------------------------
# 5-1 / Propagation Invariant: Pipeline default(20) -> Retriever까지 그대로 전달
# ---------------------------------------------------------------------------
...
def test_pipeline_default_propagates_20_to_retriever(patched_pipeline):
    ...
    assert all(k == 20 for k in patched_pipeline.last_k_values), (
        f"Pipeline 기본 호출에서 Retriever까지 전달된 k 값이 20이 아닙니다: {patched_pipeline.last_k_values}"
    )


def test_pipeline_explicit_override_not_overwritten_by_default(patched_pipeline):
    """Production Default를 20으로 올린 것이 명시적 k_per_query=5 호출을 무시하면 안 된다."""
    ...
    assert all(k == 5 for k in patched_pipeline.last_k_values), (
        f"Explicit override(k_per_query=5)가 Production Default(20)에 의해 덮어써졌습니다: {patched_pipeline.last_k_values}"
    )
...
def test_signature_defaults_are_exactly_20():
    assert inspect.signature(spec_retriever.retrieve_for_requirement).parameters["k_per_query"].default == 20
    assert inspect.signature(pipeline.retrieve_and_generate).parameters["k_per_query"].default == 20
